 ...
 * @returns {{nodes: Array, edges: Array, faces: Array}}
 */
function generateIcosahedron()
{
    ...
    for (i = 0; i < edges.length; ++i)
        for (j = 0; j < edges[i].n.length; ++j)
            nodes[j].e.push(i);

    // Push faces to node
    for (i = 0; i < faces.length; ++i)
        for (j = 0; j < faces[i].n.length; ++j)
            nodes[j].f.push(i);
...
/**
 * Generate an icosahedron with subdivision
 * @param degree
 * @returns {{nodes: Array, edges: Array, faces: Array}}
 */
...
function setup() {
    ...
    shape = generateSubdividedIcosahedron(9);
    console.log(shape);
}
...
function draw(){
    ...
    camera(sin(frameCount * 0.01) * 100, cos(frameCount * 0.01) * 100, sin(frameCount * 0.01) * 100);
    var tempEdge;
    for (i = 0; i < shape.edges.length; i++) {
        tempEdge = shape.edges[i];
        drawLine(shape.nodes[tempEdge.n[0]].p, shape.nodes[tempEdge.n[1]].p);
    }
}
...
function drawLine(p1, p2) {
    line(p1.x * 200, p1.y * 200, p1.z * 200, p2.x * 200, p2.y * 200, p2.z * 200);
}